use std::{io::{self, BufRead}, fs::File, path::Path, collections::LinkedList};
...
fn main() {
    ...
    let words : [[u8;5]; 2315] = words_to_arr();
    ...
    while run {
        if possibilities.len() == 0 { println!("no remaining possibilities!"); break; }
        let mut input = String::new();
        let guess:u16 = guess_word(&words, &possibilities).0;
        println!("Guessing...: {0}", u8arr_to_string(&words[guess as usize]));
        println!("Pattern: ");
        io::stdin().read_line(&mut input).expect("error");
        input=input.trim().to_string();
        if input.eq("end") {run = false;}
        else if input.eq("poss") {println!("{0} possabilities: {1:?}",possibilities.len(),possibilities);}
        else {possibilities = update_poss(&words, &possibilities,guess, get_pattern(&input));}
    }
    println!("ended!");
}

//takes words from 5letterhiddenwords.txt and stores into array
fn words_to_arr() -> [[u8;5]; 2315]{
    let mut res: [[u8;5]; 2315] = [[0;5]; 2315];
    let mut line_number : usize = 0;
    if let Ok(lines) = read_lines("5letterhiddenwords.txt") {
        // Consumes the iterator, returns an (Optional) String
        for line in lines {
            if let Ok(ip) = line {
                for(i,c) in ip.chars().enumerate(){
                    res[line_number][i] = (c as u8) - 97;
                }
                line_number+=1;
            }
        }
...
fn update_poss(words: &[[u8;5]; 2315], poss: &LinkedList<u16>, guessed_word: u16, pattern: u8) -> LinkedList<u16> {
...
fn evaluate(words: &[[u8;5]; 2315], guess: u16, poss: &LinkedList<u16>) -> f32{
    ...
    //actually does the funny information theory math 
    let mut res: f32 = 0.0;
...
fn guess_word(words: &[[u8;5]; 2315], poss: &LinkedList<u16>) -> (u16, f32) {
    let mut best_eval: f32 = 0.0;
    let mut best_found:u16 = 0;

    //random number i made up, if more than 50 remaining then finds the word with best eval, otherwise find POSSIBLE word with best eval
    if poss.len()>50{
        for i in 0..2315{
        ...
        for &i in poss{//ctrl c ctrl v lmao too lazy to make another method
            let eval : f32 = evaluate(&words, i, &poss);
            if eval>=best_eval {
                best_eval = eval;
                best_found = i;
...
fn gen_pattern(guess: &[u8], answer: &[u8]) -> u8{
    ...
    for i in (0..5).rev(){
        yellows*=3;
        if guess[i] != answer[i] {
            if chars_in_answer[guess[i] as usize] < chars_in_guess[guess[i] as usize] {chars_in_guess[guess[i] as usize] -= 1;}
            else {yellows += 1;}
        }
    }
    res + yellows
